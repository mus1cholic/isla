...
class StaticToolSessionReader final : public ToolSessionReader {
  public:
    explicit StaticToolSessionReader(absl::StatusOr<std::string> result)
...
TEST(ExpandMidTermToolTest, ExecuteRequiresSessionReaderWhenWired) {
    const ExpandMidTermTool tool;
...
TEST(ExpandMidTermToolTest, ExecuteReturnsExpandedDetailFromSessionReader) {
    const ExpandMidTermTool tool;
    auto reader = std::make_shared<StaticToolSessionReader>(std::string("full tier1 detail"));
    ...
    EXPECT_EQ(result->call_id, "call_3");
    EXPECT_EQ(result->tool_name, "expand_mid_term");
    EXPECT_EQ(result->output_text, "full tier1 detail");
