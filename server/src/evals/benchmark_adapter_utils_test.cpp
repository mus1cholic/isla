...
TEST(BenchmarkAdapterUtilsTest, ParseBenchmarkTimestampSupportsDateOnlyAndIso8601) {
    ...
    const absl::StatusOr<Timestamp> locomo_timestamp =
        ParseBenchmarkTimestamp("1:56 pm on 8 May, 2023", "question_date");
    ...
    ASSERT_TRUE(locomo_timestamp.ok()) << locomo_timestamp.status();
    ...
    EXPECT_EQ(*locomo_timestamp, Ts("2023-05-08T13:56:00Z"));
}
