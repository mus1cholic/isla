...
void PrintUsage() {
    std::cout << "Usage: locomo_eval --dataset=PATH [--output_dir=PATH] [--case_id=CASE_ID] "
...
int main(int argc, char** argv) {
    ...
    isla::server::evals::LoCoMoBenchmarkRunConfig config;
    for (int i = 1; i < argc; ++i) {
        ...
        std::cerr << "Unsupported flag for locomo_eval: " << arg << "\n";
        PrintUsage();
        return EXIT_FAILURE;
    }
    ...
    const absl::StatusOr<isla::server::evals::MemoryBenchmarkReport> report =
        isla::server::evals::RunLoCoMoBenchmark(std::move(config));
    if (!report.ok()) {
