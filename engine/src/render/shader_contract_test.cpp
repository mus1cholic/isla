...
TEST(ShaderContractTests, ModelRendererAppliesPerObjectMaterialStateSelectionContract) {
    ...
    EXPECT_TRUE(absl::StrContains(renderer_source, "material.blend_mode == MaterialBlendMode::AlphaBlend"))
    ...
    EXPECT_TRUE(absl::StrContains(renderer_source, "const bool alpha_cutout_enabled = alpha_cutoff >= 0.0F;"))
        << "MASK cutoff behavior should be evaluated per object/material";
    EXPECT_TRUE(absl::StrContains(renderer_source, "render_state_base | cull_state_for_material(material.cull_mode)"))
