...
class AnimatedGltfTest : public ::testing::Test {
  protected:
    ...
    static AnimatedNode make_trs_node(int parent_index, const Vec3& position = Vec3{}) {
        AnimatedNode node;
        node.parent_index = parent_index;
        node.bind_local_transform.position = position;
        node.bind_local_matrix = Mat4::from_position_scale_quat(node.bind_local_transform.position,
                                                                node.bind_local_transform.scale,
                                                                node.bind_local_transform.rotation);
        node.uses_trs = true;
        return node;
    }

    static AnimatedNode make_matrix_node(int parent_index, const Mat4& matrix) {
        AnimatedNode node;
        node.parent_index = parent_index;
        node.bind_local_matrix = matrix;
        node.uses_trs = false;
        return node;
    }
...
TEST_F(AnimatedGltfTest, LoadsAndEvaluatesAnimatedNonJointHierarchyFromGltf) {
    const std::filesystem::path temp_dir = make_temp_dir();
    ...
    const std::size_t anim_translation_offset = buffer.size();
    for (float value : { 0.0F, 0.0F, 0.0F, 2.0F, 0.0F, 0.0F }) {
        append_f32(buffer, value);
    }
    const std::size_t anim_translation_length = buffer.size() - anim_translation_offset;
...

    {
        ...
        gltf << "{\n"
             ...
             << "    {\"name\": \"joint0\", \"translation\": [1.0, 0.0, 0.0]},\n"
             << "    {\"mesh\": 0, \"skin\": 0}\n"
             << "  ],\n"
    ...
    const AnimatedGltfLoadResult loaded = load_from_file(gltf_path.string());
    ASSERT_TRUE(loaded.ok) << loaded.error_message;
    ASSERT_EQ(loaded.asset.nodes.size(), 3U);
    ASSERT_EQ(loaded.asset.joint_node_indices.size(), 1U);
    EXPECT_EQ(loaded.asset.joint_node_indices[0], 1U);

    EvaluatedPose pose;
    ASSERT_TRUE(assert_evaluate_clip_pose_ok(loaded.asset, 0.5F, pose, nullptr));
...
TEST_F(AnimatedGltfTest, EvaluatesAnimatedNonJointAncestors) {
    AnimatedGltfAsset asset;
    asset.skeleton.joints.push_back(SkeletonJoint{ .parent_index = -1 });
    asset.nodes.push_back(make_trs_node(-1));
    asset.nodes.push_back(make_trs_node(0, Vec3{ .x = 1.0F, .y = 0.0F, .z = 0.0F }));
    asset.joint_node_indices = { 1U };
    asset.bind_local_transforms.push_back(asset.nodes[1].bind_local_transform);

    AnimationClip clip;
    clip.duration_seconds = 2.0F;
    clip.node_tracks.resize(2U);
    clip.node_tracks[0].translations.push_back(
        Vec3Keyframe{ .time_seconds = 0.0F, .value = Vec3{ .x = 0.0F, .y = 0.0F, .z = 0.0F } });
    clip.node_tracks[0].translations.push_back(
        Vec3Keyframe{ .time_seconds = 2.0F, .value = Vec3{ .x = 4.0F, .y = 0.0F, .z = 0.0F } });
    asset.clips.push_back(std::move(clip));
...
TEST_F(AnimatedGltfTest, EvaluatesAnimatedIntermediateNonJointNodesBetweenJoints) {
    AnimatedGltfAsset asset;
    asset.skeleton.joints.push_back(SkeletonJoint{ .parent_index = -1 });
    asset.skeleton.joints.push_back(SkeletonJoint{ .parent_index = 0 });
    asset.nodes.push_back(make_trs_node(-1));
    asset.nodes.push_back(make_trs_node(0, Vec3{ .x = 2.0F, .y = 0.0F, .z = 0.0F }));
    asset.nodes.push_back(make_trs_node(1, Vec3{ .x = 1.0F, .y = 0.0F, .z = 0.0F }));
    asset.joint_node_indices = { 0U, 2U };
    asset.bind_local_transforms = {
        asset.nodes[0].bind_local_transform,
        asset.nodes[2].bind_local_transform,
    };

    AnimationClip clip;
    clip.duration_seconds = 2.0F;
    clip.node_tracks.resize(3U);
    clip.node_tracks[1].translations.push_back(
        Vec3Keyframe{ .time_seconds = 0.0F, .value = Vec3{ .x = 2.0F, .y = 0.0F, .z = 0.0F } });
    clip.node_tracks[1].translations.push_back(
        Vec3Keyframe{ .time_seconds = 2.0F, .value = Vec3{ .x = 4.0F, .y = 0.0F, .z = 0.0F } });
    asset.clips.push_back(std::move(clip));
    ...
    ASSERT_EQ(pose.global_joint_matrices.size(), 2U);
    EXPECT_NEAR(pose.global_joint_matrices[0].elements[12], 0.0F, 1.0e-4F);
    EXPECT_NEAR(pose.global_joint_matrices[1].elements[12], 4.0F, 1.0e-4F);
}

TEST_F(AnimatedGltfTest, SupportsStaticMatrixAuthoredNonJointAncestors) {
    AnimatedGltfAsset asset;
    asset.skeleton.joints.push_back(SkeletonJoint{ .parent_index = -1 });
    asset.nodes.push_back(
        make_matrix_node(-1, Mat4::from_position_scale_quat(Vec3{ .x = 2.0F, .y = 0.0F, .z = 0.0F },
...
TEST_F(AnimatedGltfTest, RejectsMatrixAuthoredAnimatedNonJointNodes) {
    AnimatedGltfAsset asset;
    asset.skeleton.joints.push_back(SkeletonJoint{ .parent_index = -1 });
    asset.nodes.push_back(make_matrix_node(
        -1, Mat4::from_position_scale_quat(Vec3{}, Vec3{ .x = 1.0F, .y = 1.0F, .z = 1.0F },
                                           Quat::identity())));
    asset.nodes.push_back(make_trs_node(0));
    asset.joint_node_indices = { 1U };
    asset.bind_local_transforms.push_back(asset.nodes[1].bind_local_transform);
    ...
    clip.node_tracks.resize(2U);
    clip.node_tracks[0].translations.push_back(
        Vec3Keyframe{ .time_seconds = 0.0F, .value = Vec3{} });
    clip.node_tracks[0].translations.push_back(
        Vec3Keyframe{ .time_seconds = 1.0F, .value = Vec3{ .x = 1.0F, .y = 0.0F, .z = 0.0F } });
    asset.clips.push_back(std::move(clip));
    ...
    EvaluatedPose pose;
    std::string error;
    EXPECT_FALSE(evaluate_clip_pose(asset, 0U, 0.5F, pose, &error));
    EXPECT_NE(error.find("matrix-authored animated nodes"), std::string::npos);
}

TEST_F(AnimatedGltfTest, RejectsNodeHierarchyCycles) {
    AnimatedGltfAsset asset;
    asset.skeleton.joints.push_back(SkeletonJoint{ .parent_index = -1 });
    asset.nodes.push_back(make_trs_node(1));
    asset.nodes.push_back(make_trs_node(0));
    ...
    EvaluatedPose pose;
    std::string error;
    EXPECT_FALSE(evaluate_clip_pose(asset, 0U, 0.25F, pose, &error));
    EXPECT_EQ(error, "node hierarchy contains parent cycle");
}

TEST_F(AnimatedGltfTest, RejectsOutOfRangeJointNodeMapping) {
    AnimatedGltfAsset asset;
    asset.skeleton.joints.push_back(SkeletonJoint{ .parent_index = -1 });
    asset.nodes.push_back(make_trs_node(-1));
    asset.joint_node_indices = { 2U };
    asset.bind_local_transforms.push_back(asset.nodes[0].bind_local_transform);

    AnimationClip clip;
    clip.duration_seconds = 1.0F;
    clip.node_tracks.resize(1U);
    asset.clips.push_back(std::move(clip));

    EvaluatedPose pose;
    std::string error;
    EXPECT_FALSE(evaluate_clip_pose(asset, 0U, 0.25F, pose, &error));
    EXPECT_EQ(error, "joint node index out of range");
}
